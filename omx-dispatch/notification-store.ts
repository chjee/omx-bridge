...
export class NotificationStore<TJob extends NotificationStoreJob> {
  ...
  private async withFileLock<T>(operation: () => Promise<T>): Promise<T> {
    ...
    while (!lockHandle) {
      try {
        const acquired = await fs.open(this.lockPath, "wx", PRIVATE_FILE_MODE);
        try {
  ...
  private async ensureDirectory(): Promise<void> {
    ...
    await assertNoSymlinkComponents(directory, "Notification store directory");
    const existingDirectory = await lstatIfExists(directory);
    if (existingDirectory?.isSymbolicLink()) {
      throw new Error(`Notification store directory must not be a symbolic link: ${directory}`);
    }
    if (existingDirectory && !existingDirectory.isDirectory()) {
      throw new Error(`Notification store parent is not a directory: ${directory}`);
    }
    ...
    await assertNotSymlink(this.options.storePath, "Notification store");
    await assertNotSymlink(this.lockPath, "Notification store lock");
  }
